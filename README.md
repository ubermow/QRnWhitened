...
Crucially, the AI-derived min-entropy ($H_{\infty}^{\text{AI}}$) was slightly *less conservative* than the rigorous NIST SP 800-90B bound. This indicates that the Attention-LSTM failed to identify exploitable structure in the raw hardware data beyond what classical estimators had already characterized. Consistent with findings by Gao *et al.* (2025) and Li *et al.* (2020), DL-based estimators converge near, but do not systematically exceed, classical statistical bounds for raw, unconditioned QRNG outputs. This is a highly positive security indicator: it confirms the mathematical conservatism of the classical bounds, as even a non-linear universal function approximator finds less exploitable regularity in the raw physical bitstream than the theoretical worst-case assumes.
...
*Clone and compile:*
```bash
git clone [https://github.com/usnistgov/SP800-90B_EntropyAssessment.git](https://github.com/usnistgov/SP800-90B_EntropyAssessment.git)
...
```bash
./ea_non_iid -v -c /path/to/your/unpacked_bitstream.bin 
```

### NIST SP 800-22 (Statistical Test Suite)
*Used to verify the cryptographic uniformity of the final FFT-whitened output.*

1. Download the suite from the Official NIST CSRC Website.
2. Extract the folder and compile the Linux binary:
```bash
cd sts-2.1.2
make clean
make
